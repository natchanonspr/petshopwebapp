...
const seedPayments = [
  { id: 1, type: 'promptpay_phone', name: 'พร้อมเพย์เบอร์โทร', detail: '081-234-5678', icon: 'fa-mobile-screen-button', default: true },
  { id: 2, type: 'promptpay_qr', name: 'พร้อมเพย์ QR', detail: 'QR Code พร้อมเพย์', icon: 'fa-qrcode', default: false },
]
...
function Addresses() {
  ...
  const provinceName = (id) =>
    geography.provinces.find(
      (item) => String(item.id) === String(id)
    )?.name || ''

  const districtName = (id) =>
    geography.districts.find(
      (item) => String(item.id) === String(id)
    )?.name || ''

  const subdistrictName = (id) =>
    ...
    const districtId =
      ...
      geography.districts.find(
        (item) =>
          ...
          String(item.provinceId) === String(provinceId)
      )?.id ||
      ''
  ...
  const remove = async (id) => {
    try {
      ...
      setError(err.message || 'ไม่สามารถลบที่อยู่ได้')
    } finally {
      setDeletingId(null)
    }
  }
  ...
  return (
    <PageShell title="ที่อยู่จัดส่ง" icon="fa-location-dot">

      {/* เพิ่มที่อยู่ */}
      <button
        type="button"
        onClick={openAdd}
        className="flex w-full items-center gap-3 rounded-3xl border border-dashed border-orange-300 bg-orange-50 p-4 text-left transition active:scale-[0.99]"
      >
        <span className="grid size-11 place-items-center rounded-2xl bg-white text-orange-500 shadow-sm">
          <i className="fa-solid fa-plus" />
        </span>

        <span className="flex-1">
          <strong className="block text-sm font-extrabold text-gray-800">
            เพิ่มที่อยู่ใหม่
          </strong>
      ...
      {/* Error */}
      {error && (
        <div className="mt-3 rounded-2xl bg-red-50 px-4 py-3 text-xs font-bold text-red-500">
          {error}
        </div>
      )}
      ...
      <div className="mt-5 space-y-3">
        ...
        {loadingAddresses ? (
          <div className="rounded-3xl bg-white p-8 text-center shadow-sm">
            <i className="fa-solid fa-spinner fa-spin text-xl text-orange-500" />
          ...
          <div className="rounded-3xl bg-white p-8 text-center shadow-sm">
            ...
            <p className="mt-1 text-xs text-gray-400">
              เพิ่มที่อยู่เพื่อให้สามารถสั่งซื้อสินค้าได้
            </p>
          </div>
        ) : (
          addresses.map((address) => (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="grid size-10 shrink-0 place-items-center rounded-full bg-orange-50 text-orange-500">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    <h2 className="m-0 text-sm font-extrabold text-gray-900">
                      {address.recipient}
                    </h2>

                    {address.default && (
                      <span className="rounded-full bg-orange-100 px-2 py-1 text-[10px] font-extrabold text-orange-600">
                        หลัก
                      </span>
                    )}
                  </div>

                  <p className="m-0 mt-1 text-xs font-medium text-gray-500">
                    {address.phone}
                  </p>
              ...
              <div className="mt-3 flex items-center justify-end gap-2 border-t border-gray-100 pt-3">
                ...
                {!address.default && (
                  <button
                    ...
                    className="rounded-full bg-slate-100 px-3 py-2 text-[11px] font-bold text-slate-800"
                  >
                    ตั้งเป็นหลัก
                  </button>
                )}
                ...
                <button
                  ...
                  className="rounded-full bg-orange-50 px-3 py-2 text-[11px] font-bold text-orange-600"
                >
        ...
        >
          <div className="w-full max-w-[380px] rounded-[28px] bg-white p-6 text-center shadow-[0_20px_60px_rgba(15,23,42,0.22)]">
            ...
            <p className="mt-2 text-sm text-slate-500">
              คุณต้องการบันทึกที่อยู่จัดส่งนี้หรือไม่?
            </p>
            ...
            <div className="mt-6 flex gap-2">
              ...
              <button
                ...
                onClick={confirmSave}
                disabled={saving}
                className="h-12 flex-1 rounded-2xl bg-orange-500 text-sm font-extrabold text-white disabled:opacity-50"
              >
                {saving ? (
      ...
      {showSuccess && (
        ...
        >
          <div
            ...
            style={{
              animation:
                'addressCardIn .48s cubic-bezier(.2,.8,.2,1) both',
            }}
          >
            <style>{`
            ...
            <p className="mt-2 text-sm text-slate-500">
              ที่อยู่จัดส่งของคุณถูกบันทึกเรียบร้อย
            </p>
...
function PaymentForm({ value, onChange, onSave, onCancel, editing }) {
  ...
  return <form onSubmit={onSave} className="rounded-3xl bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><h2 className="m-0 text-base font-extrabold text-gray-900">{editing ? 'แก้ไขวิธีชำระเงิน' : 'เพิ่มวิธีชำระเงิน'}</h2><p className="m-0 mt-1 text-xs text-gray-400">ข้อมูลจะถูกใช้ในหน้าชำระเงิน</p></div><span className="grid size-10 place-items-center rounded-full bg-orange-50 text-orange-500"><i className="fa-solid fa-wallet" /></span></div><div className="grid grid-cols-2 gap-2">{[['promptpay_phone', 'พร้อมเพย์เบอร์โทร', 'fa-mobile-screen-button'], ['promptpay_qr', 'พร้อมเพย์ QR', 'fa-qrcode']].map(([id, label, icon]) => <button key={id} type="button" onClick={() => onChange({ ...value, type: id })} className={`rounded-2xl border p-3 text-left ${type === id ? 'border-orange-500 bg-orange-50' : 'border-gray-100 bg-white'}`}><i className={`fa-solid ${icon} mr-2 ${type === id ? 'text-orange-500' : 'text-gray-400'}`} /><span className="text-xs font-bold">{label}</span></button>)}</div>{type === 'promptpay_phone' && <label className="mt-4 block"><span className="mb-1.5 block text-xs font-bold text-gray-600">เบอร์โทรศัพท์พร้อมเพย์ 10 หลัก</span><input required inputMode="tel" maxLength={10} value={value.detail || ''} onChange={(e) => onChange({ ...value, detail: e.target.value.replace(/\D/g, '').slice(0, 10) })} placeholder="0812345678" className="h-11 w-full rounded-xl bg-gray-100 px-3 text-sm outline-none focus:ring-2 focus:ring-orange-200" /></label>}{type === 'promptpay_qr' && <div className="mt-4 rounded-2xl bg-gray-50 p-4 text-xs leading-5 text-gray-500">ใช้ QR Code พร้อมเพย์สำหรับรับชำระเงิน</div>}<label className="mt-4 flex items-center justify-between rounded-2xl bg-orange-50 px-4 py-3 text-xs font-bold text-orange-700">ตั้งเป็นวิธีหลัก<input type="checkbox" checked={Boolean(value.default)} onChange={(e) => onChange({ ...value, default: e.target.checked })} className="size-4 accent-orange-500" /></label><div className="mt-5 flex gap-2"><button type="button" onClick={onCancel} className="h-11 flex-1 rounded-full bg-gray-100 text-sm font-bold text-gray-600">ยกเลิก</button><button type="submit" className="h-11 flex-1 rounded-full bg-orange-500 text-sm font-bold text-white">{editing ? 'บันทึก' : 'เพิ่มวิธีชำระเงิน'}</button></div></form>
}

function Payments() {
  const [payments, setPayments] = useState(() => {
    const stored = readStorage(PAYMENT_STORAGE_KEY, seedPayments)
    const allowed = stored.filter((payment) => ['promptpay_phone', 'promptpay_qr'].includes(payment.type))
    return allowed.length ? allowed : seedPayments
  }), [showForm, setShowForm] = useState(false), [editingId, setEditingId] = useState(null), [form, setForm] = useState({ type: 'promptpay_phone', detail: '', default: false })
  ...
  const openAdd = () => { setEditingId(null); setForm({ type: 'promptpay_phone', detail: '', default: payments.length === 0 }); setShowForm(true) }
  const openEdit = (payment) => { setEditingId(payment.id); setForm({ type: payment.type, detail: payment.detail || '', default: Boolean(payment.default) }); setShowForm(true) }
  const display = (payment) => payment.detail || (payment.type === 'promptpay_qr' ? 'QR Code พร้อมเพย์' : '-')
  const save = (event) => { event.preventDefault(); const detail = form.type === 'promptpay_qr' ? 'QR Code พร้อมเพย์' : form.detail.trim(); if (form.type === 'promptpay_phone' && !/^\d{10}$/.test(detail)) return; const names = { promptpay_phone: 'พร้อมเพย์เบอร์โทร', promptpay_qr: 'พร้อมเพย์ QR' }, icons = { promptpay_phone: 'fa-mobile-screen-button', promptpay_qr: 'fa-qrcode' }; const item = { id: editingId ?? Date.now(), type: form.type, name: names[form.type], detail, icon: icons[form.type], default: form.default }; let next = editingId ? payments.map((p) => p.id === editingId ? item : p) : [...payments, item]; if (item.default || next.length === 1) next = next.map((p) => ({ ...p, default: p.id === item.id })); else if (!next.some((p) => p.default)) next = next.map((p, i) => ({ ...p, default: i === 0 })); persist(next); setShowForm(false); setEditingId(null) }
