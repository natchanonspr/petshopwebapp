...
import { getRecommendationHistory } from '../../api/ai.js'
...
export default function AdminSettings() {
  ...
  useEffect(() => {
    ...
    getRecommendationHistory()
      ...
      .catch((err) => {
        ...
        setHistoryError(
          err?.message || 'ไม่สามารถโหลดประวัติ AI ได้',
        )
      })
      .finally(() => {
        if (active) {
  ...
  return <div className="space-y-4 pb-20 md:pb-6">
    ...
    <section className="rounded-xl border border-violet-100 bg-gradient-to-br from-violet-50 via-white to-white p-4 shadow-sm">
    ...
    <Stat
      label="ตรวจไม่ถูกต้อง"
      value="-"
      icon="fa-circle-xmark"
    />

    {tab === 'rules' && <section className="rounded-xl border border-[#ececf2] bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-base font-extrabold">เงื่อนไขวิเคราะห์อาหาร</h2><p className="mt-1 text-xs text-gray-400">กำหนดข้อมูลที่ AI ต้องนำมาพิจารณาก่อน Recommendation</p></div><span className="text-xs font-bold text-violet-600">เปิด {ai.rules.filter(r => r.enabled).length} รายการ</span></div><div className="mt-4 grid gap-3 md:grid-cols-2">{ai.rules.map(rule => <div key={rule.id} className="flex items-center gap-3 rounded-xl border border-gray-100 p-4"><span className="grid size-10 place-items-center rounded-xl bg-violet-50 text-violet-600"><i className="fa-solid fa-sliders" /></span><div className="flex-1"><div className="text-sm font-bold">{rule.label}</div><p className="mt-1 text-xs text-gray-400">{rule.description}</p></div><button onClick={() => toggleRule(rule.id)} className={`h-6 w-11 rounded-full p-1 transition ${rule.enabled ? 'bg-violet-600' : 'bg-gray-200'}`}><span className={`block size-4 rounded-full bg-white transition ${rule.enabled ? 'translate-x-5' : ''}`} /></button></div>)}</div></section>}

    {tab === 'nutrition' && <section className="rounded-xl border border-[#ececf2] bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-base font-extrabold">ฐานข้อมูลโภชนาการสัตว์</h2><p className="mt-1 text-xs text-gray-400">ข้อมูลอ้างอิงสำหรับระบบ Recommendation</p></div><button onClick={() => setNutritionForm({ pet: 'แมว', category: '', protein: '', fat: '', note: '' })} className="rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-bold text-white"><i className="fa-solid fa-plus mr-2" />เพิ่มข้อมูล</button></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-xs"><thead><tr className="border-b border-gray-100 text-gray-400"><th className="px-3 py-3">สัตว์</th><th>หมวด</th><th>โปรตีน</th><th>ไขมัน</th><th>หมายเหตุ</th><th className="text-right">จัดการ</th></tr></thead><tbody>{ai.nutrition.map(item => <tr key={item.id} className="border-b border-gray-50"><td className="px-3 py-3 font-bold">{item.pet}</td><td>{item.category || '-'}</td><td>{item.protein || '-'}</td><td>{item.fat || '-'}</td><td className="text-gray-500">{item.note || '-'}</td><td className="text-right"><button onClick={() => setNutritionForm(item)} className="mr-2 rounded-lg bg-gray-50 px-2.5 py-1.5 font-bold text-gray-500">แก้ไข</button><button onClick={() => removeNutrition(item.id)} className="rounded-lg bg-red-50 px-2.5 py-1.5 font-bold text-red-500">ลบ</button></td></tr>)}</tbody></table></div></section>}
      ...
      <>
        ...
        {!historyLoading && !historyError && (
          <RecommendationHistory
            ...
            onReview={review}
            correctCount={history.filter((item) => item.correct === true).length}
            wrongCount={history.filter((item) => item.correct === false).length}
          />
        )}
      </>
    )}
...
function RecommendationHistory({ recommendations }) {
    ...
    <section className="rounded-xl border border-[#ececf2] bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-extrabold">
            ประวัติ AI Recommendation
          </h2>

          <p className="mt-1 text-xs text-gray-400">
            ข้อมูลที่บันทึกจากการวิเคราะห์จริง
          </p>
        </div>

        <span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-bold text-violet-600">
          ทั้งหมด {recommendations.length} รายการ
        </span>
      </div>
      ...
      <div className="mt-4 flex gap-2">
        <button
          ...
          className={`rounded-lg px-3 py-2 text-xs font-bold ${filter === 'all'
        ...
        <button
          ...
          className={`rounded-lg px-3 py-2 text-xs font-bold ${filter === 'recent'
              ? 'bg-violet-600 text-white'
              : 'bg-gray-100 text-gray-500'
            }`}
        >
      ...
      <div className="mt-4 space-y-3">
        {filtered.length === 0 && (
          <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-8 text-center text-sm text-gray-400">
            <i className="fa-solid fa-robot mb-2 text-2xl text-gray-300" />
            <div>ยังไม่มีประวัติ AI</div>
          </div>
        )}
        ...
        {filtered.map((item) => (
          <div
            key={item.id}
            className="rounded-2xl border border-gray-100 p-4 transition hover:border-violet-100 hover:bg-violet-50/20"
          >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 flex items-start gap-3">
                  {item.product_image ? (
                    ...
                    <span className="grid size-16 shrink-0 place-items-center rounded-xl bg-gray-100 text-gray-400">
...
function Metric({ icon, label, value }) { return <div className="h-full rounded-xl border border-[#ececf2] bg-white p-4 shadow-sm"><div className="grid size-10 place-items-center rounded-xl bg-violet-50 text-violet-600"><i className={`fa-solid ${icon}`} /></div><div className="mt-3 text-xs text-gray-400">{label}</div><div className="mt-1 text-xl font-extrabold">{value}</div></div> }
