...
const getPaymentLabel = (value) => {
  ...
  return (
    paymentMap[String(value).toLowerCase()] ||
    value
  )
}
...
export default function OrderDetail() {
      ...
      <main className="mx-auto flex min-h-[100dvh] w-full max-w-[430px] items-center justify-center bg-gray-50 px-5 text-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            {errorMessage ||
              'คำสั่งซื้ออาจถูกลบหรือไม่มีอยู่แล้ว'}
          </p>

          <Link
            to="/orders"
            className="mt-5 inline-flex h-11 items-center rounded-full bg-orange-500 px-6 text-sm font-bold text-white"
          >
  ...
  const address = parseAddressSnapshot(
  ...
  const handleReorder = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error.message ||
          'ไม่สามารถเพิ่มสินค้าเดิมลงตะกร้าได้',
      )
    } finally {
      setReordering(false)
    }
